...
def main(_):
    ...
    # Keep checkpoints under the same run log directory.
    ...
    try:
        reward_workers = max(1, int(reward_workers_env)) if reward_workers_env else 8
    except ValueError:
        reward_workers = 8
    executor = futures.ThreadPoolExecutor(max_workers=reward_workers)  # Async reward computation
    max_pending_rewards_env = os.getenv("REWARD_MAX_PENDING", "0")
    try:
        max_pending_rewards = max(0, int(max_pending_rewards_env))
    except ValueError:
        max_pending_rewards = 0
    if max_pending_rewards:
        logger.info("Reward throttling enabled: max_pending_rewards=%d", max_pending_rewards)
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        if is_main_process(rank):  # logging
            ...
            for k, v in gathered_rewards_dict.items():
                if "_strict_accuracy" in k or "_accuracy" in k:
                    continue
