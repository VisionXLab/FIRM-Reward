...
# Ensure project root is on sys.path when running via scripts/ path.
...
def _get_indexed_value(value, idx):
...
def _extract_reward_raw(reward_metadata, idx):
    if not reward_metadata:
        return {}
    per_sample = {}
    for score_name, meta in reward_metadata.items():
        if isinstance(meta, dict):
            meta_out = {}
            for key, value in meta.items():
                meta_out[key] = _get_indexed_value(value, idx)
            if meta_out:
                per_sample[score_name] = meta_out
        else:
            per_sample[score_name] = _get_indexed_value(meta, idx)
    return per_sample


def _save_rollout_images(images, rollout_root, epoch, batch_idx, rank):
    if images is None:
        return []
    images_cpu = images.detach().float().clamp(0, 1).cpu()
    rollout_dir = _ensure_dir(
        os.path.join(
            rollout_root,
            f"epoch_{epoch:06d}",
    ...
    paths = []
    for idx, img in enumerate(images_cpu):
        pil = Image.fromarray(
            (img.numpy().transpose(1, 2, 0) * 255).round().astype(np.uint8)
        )
        filename = f"e{epoch:06d}_b{batch_idx:06d}_r{rank:03d}_{idx:06d}.jpg"
...
def _build_reward_records(
    ...
    for idx, prompt in enumerate(prompts):
        metadata = prompt_metadata[idx] if idx < len(prompt_metadata) else {}
        image_rel_path = metadata.get("image", "")
        src_image_path = (
            os.path.abspath(os.path.join(dataset_root, image_rel_path))
            if image_rel_path
            else ""
        )
        reward_scores = {}
        for key, value in rewards.items():
            score_value = _get_indexed_value(value, idx)
            try:
                reward_scores[key] = float(score_value)
            except (TypeError, ValueError):
                reward_scores[key] = score_value
        records.append(
            {
                ...
                "rollout_image_path": rollout_paths[idx]
                if idx < len(rollout_paths)
                else "",
                "reward_scores": reward_scores,
                "reward_raw": _extract_reward_raw(reward_metadata, idx),
            }
        )
    return records


def _append_reward_records(records, json_path):
    if not records:
        return
...
def main(_):
    ...
    reward_log_root = os.path.abspath(
        os.path.join(config.logdir, config.run_name, "reward_records")
    )
    reward_rollout_root = _ensure_dir(os.path.join(reward_log_root, "rollouts"))
    reward_json_path = os.path.join(reward_log_root, f"{config.run_name}.json")
    dataset_root = os.path.abspath(config.dataset) if config.dataset else ""
    ...
    pipeline.text_encoder.to(dtype=text_encoder_dtype)
    pipeline.text_encoder_2.to(dtype=text_encoder_dtype)
    ...
    reward_fn = getattr(flow_grpo.rewards, "multi_score")(
        device,
        config.reward_fn,
        normalize_mllm_score=getattr(config, "mllm_score_normalize", True),
        normalize_mllm_score_mode=getattr(
            config, "mllm_score_normalize_mode", "div5"
        ),
        normalize_mllm_score_range=getattr(
            config, "mllm_score_normalize_range", (0.0, 1.0)
        ),
        exec_consistency_guard=getattr(config, "mllm_exec_consistency_guard", False),
        exec_consistency_exec_lt=getattr(
            config, "mllm_exec_consistency_exec_lt", 3.0
        ),
        exec_consistency_cons_ge=getattr(
            config, "mllm_exec_consistency_cons_ge", 4.0
        ),
        use_total_score=getattr(config, "mllm_use_total_score", False),
        total_score_w1=getattr(config, "mllm_total_score_w1", 1.0),
        total_score_w2=getattr(config, "mllm_total_score_w2", 1.0),
        total_score_mode=getattr(config, "mllm_total_score_mode", "exec_consistency"),
    )  # Pass device
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            rollout_paths = _save_rollout_images(
                images, reward_rollout_root, epoch, i, rank
            )
        ...
        for sample_item in tqdm(
            ...
            reward_records = _build_reward_records(
                sample_item.get("prompts", []),
                sample_item.get("prompt_metadata", []),
                sample_item.get("rollout_paths", []),
                rewards,
                reward_metadata,
                dataset_root,
                config.run_name,
            ...
            if world_size > 1 and hasattr(dist, "gather_object"):
                gathered_records = [None for _ in range(world_size)] if rank == 0 else None
                dist.gather_object(reward_records, gathered_records, dst=0)
                if rank == 0:
                    merged_records = []
                    for part in gathered_records:
                        if part:
                            merged_records.extend(part)
                    _append_reward_records(merged_records, reward_json_path)
            else:
                if is_main_process(rank):
                    _append_reward_records(reward_records, reward_json_path)

            sample_item["rewards"] = {
                k: torch.as_tensor(v, device=device).float() for k, v in rewards.items()
            }
            del sample_item["rewards_future"]
            sample_item.pop("prompts", None)
            sample_item.pop("prompt_metadata", None)
            sample_item.pop("rollout_paths", None)
            sample_item.pop("sample_batch_idx", None)
