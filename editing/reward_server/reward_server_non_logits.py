...
# Global variables
DEFAULT_SCORE_IDX = [15, 16, 17, 18, 19, 20]
score_idx_env = os.getenv("REWARD_SCORE_IDX")
score_idx = (
    [int(token_id) for token_id in score_idx_env.split(",") if token_id.strip()]
    if score_idx_env
    else DEFAULT_SCORE_IDX
)
workers = []
DEFAULT_MODEL_PATH = (
    "Qwen/Qwen2.5-VL-7B-Instruct"
)
MODEL_PATH = os.getenv("REWARD_MODEL_PATH", DEFAULT_MODEL_PATH)
TRUST_REMOTE_CODE = os.getenv("REWARD_TRUST_REMOTE_CODE", "true").lower() in ("1", "true", "yes")
...
class ModelWorker:
    ...
    def _transformers_evaluate(self, messages, max_tokens=3, max_score=5):
        try:
            ...
            if outputs.logits:
                first_token_logits = outputs.logits[0][-1, score_idx]
