...
def get_config():
    ...
    # normalize mllm reward scores from 0-5 to 0-1 when using mllm-based rewards.
    config.mllm_score_normalize = True
    # normalization mode for mllm scores: "div5" (default), "range_1_5", "range_1_5_0.2_1".
    config.mllm_score_normalize_mode = "div5"
    # target range for fixed-range normalization when mode is "range_1_5" or "range_1_5_range".
    config.mllm_score_normalize_range = (0.0, 1.0)
    # use TotalScore = Execution * (w1 + w2 * Consistency) as the final reward.
    config.mllm_use_total_score = False
    config.mllm_total_score_w1 = 0.6
    config.mllm_total_score_w2 = 0.4
    # total score mode: "exec_consistency" (default) or "execution_only" for ablations.
    config.mllm_total_score_mode = "exec_consistency"
    config.save_dir = ""
