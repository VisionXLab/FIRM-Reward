...
def _get_config(base_model="kontext", n_gpus=1, gradient_step_per_epoch=1, reward_fn={}, name=""):
    ...
    config.run_name = f"nft_{base_model}_{name}_qwen3_vl_8b_2_64_base_normalize_0211"
    config.logdir = f"logs/nft/qwen3_vl_8b_sft/{base_model}/{name}"
    config.save_dir = f"logs/nft/qwen3_vl_8b_sft/{base_model}/{name}"
    config.reward_fn = reward_fn
    config.mllm_exec_consistency_guard = True
    config.mllm_exec_consistency_exec_lt = 2.0
